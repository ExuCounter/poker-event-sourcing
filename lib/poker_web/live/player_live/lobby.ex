      ...
      <div class="flex flex-1 min-h-0">
        ...
        <div class="flex-1 flex items-center justify-center p-8">
          <div class="w-[620px] h-[360px] relative">
            <!-- Felt oval -->
            <div class="absolute inset-0 rounded-[50%]" style="background: linear-gradient(180deg, oklch(20% 0.03 28), oklch(12% 0.02 28)); box-shadow: 0 40px 80px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05)"></div>
            <div class="absolute inset-4 rounded-[50%]" style="background: radial-gradient(ellipse at center, oklch(38% 0.08 160) 0%, oklch(28% 0.06 160) 55%, oklch(20% 0.04 160) 100%); box-shadow: inset 0 0 0 1px oklch(76% 0.13 85 / 0.3), inset 0 4px 24px rgba(0,0,0,0.5)"></div>
            <div class="absolute inset-[26px] rounded-[50%] border border-[var(--pkr-accent)]/20"></div>
            ...
            <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center z-10">
              <div class="font-[family-name:var(--pkr-font-mono)] text-[11px] uppercase tracking-[0.12em] text-[var(--pkr-ink-3)] mb-2">
                <%= if @lobby.status == :waiting, do: "WAITING TO START", else: String.upcase(to_string(@lobby.status)) %>
              </div>
            ...
            <%= for pos <- @seat_positions do %>
              ...
              <div class="absolute z-10" style={"left: #{pos.x}%; top: #{pos.y}%; transform: translate(-50%, -50%)"}>
                <%= if participant do %>
                  <div class={"flex items-center gap-2 px-2.5 py-1.5 rounded-full backdrop-blur-md bg-black/60 " <>
                    ...
                    <div class="text-xs leading-tight">
                      <div class="text-[var(--pkr-ink-1)]">{participant.nickname || String.split(participant.email, "@") |> hd()}</div>
                    </div>
                  </div>
                <% else %>
        ...
        <aside class="w-[360px] border-l border-[var(--pkr-line)] p-5 flex flex-col gap-3.5 overflow-auto">
          <div>
            <div class="font-[family-name:var(--pkr-font-mono)] text-[11px] uppercase tracking-[0.12em] text-[var(--pkr-ink-3)] mb-1.5">TABLE</div>
            <h2 class="font-[family-name:var(--pkr-font-display)] text-[28px] leading-none text-[var(--pkr-ink-1)]">NL Hold'em</h2>
          ...
          <div class="rounded-xl border border-[var(--pkr-line)] bg-[var(--pkr-bg-1)] p-3.5">
            <div class="font-[family-name:var(--pkr-font-mono)] text-[10px] uppercase tracking-[0.1em] text-[var(--pkr-ink-3)] mb-2.5">RULES</div>
            <div class="space-y-0">
          ...
          <%= cond do %>
            <% user_has_joined?(@lobby.participants, @user_id) -> %>
              <.link navigate={~p"/tables/#{@lobby.id}/game"} class="block w-full text-center py-3.5 rounded-xl text-sm font-medium bg-[var(--pkr-accent)] text-[var(--pkr-bg-0)] hover:brightness-110 transition-all">
                Enter Game
              </.link>
            <% @lobby.seated_count < @lobby.seats_count -> %>
              <button phx-click="join_table" class="w-full py-3.5 rounded-xl text-sm font-medium bg-[var(--pkr-accent)] text-[var(--pkr-bg-0)] hover:brightness-110 transition-all cursor-pointer">
                Take seat &amp; join
              </button>
            <% true -> %>
          ...
          <.link navigate={~p"/tables/#{@lobby.id}/game"} class="block w-full text-center py-3 rounded-xl text-[13px] text-[var(--pkr-ink-2)] border border-[var(--pkr-line)] hover:bg-[var(--pkr-bg-2)] transition-all">
            Watch from rail
          </.link>

          <%= if @lobby.status in [:waiting, :live] && @lobby.seated_count >= 2 && @lobby.creator_id == @user_id do %>
            <button phx-click="start_table" class="w-full py-3 rounded-xl text-[13px] font-medium text-[var(--pkr-positive)] border border-[var(--pkr-positive)]/40 hover:bg-[var(--pkr-positive)]/10 transition-all cursor-pointer">
              Start Game
            </button>
          <% end %>
        </aside>
